...
export const fileReadSchema = z.object({
  path: z
    .string()
    .describe("Path to file (relative paths resolve against cwd)"),
  offset: z
    .number()
    .int()
    .min(1)
    .optional()
    .describe(
      "Line number to start reading from, starting with 1; omit for one shot full file read ",
    ),
  limit: z
    .number()
    .int()
    .min(1)
    .optional()
    .describe("Max lines to return; omit for default cap (1000)"),
})
...
export const fileReadResultSchema = z.object({
  ...
  totalLineCount: z
    ...
    .int()
    .min(0)
    .describe("Total number of lines to read in the file"),
  offset: z
    .number()
    .int()
    .min(1)
  ...
  limit: z
    ...
    .int()
    .min(1)
    .describe("Lines actually returned in this response"),
  truncated: z
    .boolean()
    .default(false)
    .describe("Whether file read is in full(false) or in batches(true)"),
  ...
  content: z.array(
    z.object({
      lineNo: z
        ...
        .int()
        .min(1)
        .describe("Current line's line number indexed from offset"),
  ...
  function: {
    ...
    execute: async (
      ...
      try {
        ...
        logger.error(
          { event: "read_file_error", toolId, resolvedPath, err: String(err) },
          "read_file failed",
        )
        return {
          tool_call_id: toolId,
...
async function readFileFromLineOffset(
  filePath: string,
  lineOffset: number,
  limit: number,
) {
